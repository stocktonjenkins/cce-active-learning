...
class DataTransform:
    ...
    def __call__(
        self, feature_data: FeatureData, idxs: Indexes | None = None
    ) -> FeatureData:
        """
        ...
        pieces = [
            self.forward(sub_data, sub_idxs)
...
class FeatureMap(DataTransform):
    ...
    def precompute(
        ...
        if self.allow_precompute_features:
            ...
            return IdentityFeatureMap(n_features=self.n_features), self(
                feature_data, idxs
            )
        else:
            # use another precomputation method that can be overridden by a subclass.
            results = [
                self.precompute_soft_(sub_data, sub_idxs)
                for sub_idxs, sub_data in feature_data.iterate(idxs)
            ]
            if len(results) == 1:
                return results[0]
    ...
    def precompute_soft_(
        self, feature_data: FeatureData, idxs: Indexes
    ) -> tuple["FeatureMap", FeatureData]:
        """
    ...
    def posterior(
        ...
        sigma: float,
        allow_kernel_space_posterior: bool = True,
    ) -> "FeatureMap":
        """
        ...
        :return: Returns a feature map that represents the Gaussian Process posterior kernel after
                 observing feature_data,
        if the noise variance is sigma^2.
        """
        if self.n_features < 0 or (
            allow_kernel_space_posterior
            and self.n_features > max(1024, 3 * len(feature_data))
        ):
            # compute the posterior in kernel space
            return KernelSpacePosteriorFeatureMap(
                feature_map=self, cond_data=feature_data, sigma=sigma
            )

        feature_matrix = self.get_feature_matrix(feature_data)
        eye = torch.eye(
            self.n_features, device=feature_matrix.device, dtype=feature_matrix.dtype
        )
        cov_matrix = feature_matrix.t().matmul(feature_matrix) + (sigma**2) * eye

        return SequentialFeatureMap(
            LinearFeatureMap(sigma * robust_cholesky_inv(cov_matrix).t()), [self]
        )
    ...
    def get_feature_matrix(
        ...
        return torch_cat(
            [
                self.get_feature_matrix_impl_(sub_data, sub_idxs)
                for sub_idxs, sub_data in feature_data.iterate(idxs)
            ],
            dim=-2,
        )
    ...
    def get_kernel_matrix(
        ...
        idxs_1: Indexes | None = None,
        idxs_2: Indexes | None = None,
    ) -> torch.Tensor:
        """
        ...
        return torch_cat(
            [
                torch_cat(
                    [
                        self.get_kernel_matrix_impl_(
                            sub_data_1, sub_data_2, sub_idxs_1, sub_idxs_2
                        )
    ...
    def get_kernel_matrix_diag_impl_(
        self, feature_data: FeatureData, idxs: Indexes
    ) -> torch.Tensor:
        """
...
class IdentityFeatureMap(FeatureMap):
    ...
    def sketch(self, n_features: int, **config) -> "FeatureMap":
        ...
        if config.get("sketch_norm", False):
            ...
            matrix /= (matrix**2).sum(dim=0, keepdim=True).sqrt() / math.sqrt(
                self.n_features
            )
...
class LinearFeatureMap(FeatureMap):
    ...
    def get_feature_matrix_impl_(
        ...
        if not isinstance(feature_data, TensorFeatureData):
            raise ValueError(
                f"feature_data must be of type ListFeatureData, but is of type {type(feature_data)}"
            )
    ...
    def sketch(self, n_features: int, **config) -> "FeatureMap":
        ...
        return SequentialFeatureMap(
            IdentityFeatureMap(n_features=self.get_n_features()), [self]
        ).sketch(n_features, **config)
...
class SequentialFeatureMap(FeatureMap):
    ...
    def __init__(self, feature_map: FeatureMap, tfms: list[Callable]):
        ...
        """
        super().__init__(
            n_features=feature_map.get_n_features(),
    ...
    def get_feature_matrix_impl_(
        ...
        feature_data = self.tfms[0](
            feature_data, idxs
        )  # or: SubsetFeatureData(feature_data, idxs)
    ...
    def sketch(self, n_features: int, **config) -> "FeatureMap":
        return SequentialFeatureMap(
            self.feature_map.sketch(n_features, **config), self.tfms
        )
...
class KernelSpacePosteriorFeatureMap(FeatureMap):
    ...
    def precompute_soft_(
        ...
        tensor = (
            self.feature_map.get_kernel_matrix(feature_data, self.cond_data, idxs)
            @ self.L_inv.t()
        )
        return PrecomputedKernelSpacePosteriorFeatureMap(
            self.feature_map
    ...
    def get_kernel_matrix_impl_(
        ...
        prior_mat = self.feature_map.get_kernel_matrix(
            feature_data_1, feature_data_2, idxs_1, idxs_2
        )
        mat_1 = self.feature_map.get_kernel_matrix(
            feature_data_1, self.cond_data, idxs_1
        )
        mat_2 = self.feature_map.get_kernel_matrix(
            feature_data_2, self.cond_data, idxs_2
        )
...
class PrecomputedKernelSpacePosteriorFeatureMap(FeatureMap):
    ...
    def get_kernel_matrix_impl_(
        ...
        if not isinstance(feature_data_1, ListFeatureData):
            raise ValueError(
                f"feature_data_1 must be of type ListFeatureData, but is of type {type(feature_data_1)}"
            )
        if not isinstance(feature_data_2, ListFeatureData):
            raise ValueError(
                f"feature_data_2 must be of type ListFeatureData, but is of type {type(feature_data_2)}"
            )
        prior_mat = self.feature_map.get_kernel_matrix(
            feature_data_1.feature_data_list[0],
            feature_data_2.feature_data_list[0],
            idxs_1,
            idxs_2,
        )
    ...
    def get_kernel_matrix_diag_impl_(
        ...
        prior_diag = self.feature_map.get_kernel_matrix_diag(
            feature_data.feature_data_list[0], idxs
        )
