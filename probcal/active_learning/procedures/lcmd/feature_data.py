...
class Indexes:
    ...
    def __init__(
        ...
        if idxs is None:
            ...
            start = (
                ...
                else idxs.start + (0 if idxs.start >= 0 else n_samples)
            )
            stop = (
                n_samples
                if idxs.stop is None
                else idxs.stop + (0 if idxs.stop >= 0 else n_samples)
            )
            if stop <= start:
    ...
    def compose(self, other: "Indexes") -> "Indexes":
        ...
        if isinstance(self.idxs, torch.Tensor):
            ...
            if isinstance(other.idxs, torch.Tensor):
                ...
                new_idxs = slice(
                    self.idxs.start + other.idxs.start,
                    self.idxs.start + other.idxs.stop,
                )
            else:
    ...
    def split_by_sizes(
        ...
        if isinstance(self.idxs, slice):
            ...
            raise NotImplementedError(
                "indexing splitted data with a list of indexes is currently not supported"
            )
    ...
    def is_all_slice(self) -> bool:
        ...
        """
        return (
            isinstance(self.idxs, slice)
            and self.idxs.start == 0
            and self.idxs.stop == self.n_samples
        )
...
class FeatureData:
    ...
    def simplify(
        self, idxs: torch.Tensor | slice | int | Indexes | None = None
    ) -> "FeatureData":
        """
    ...
    def simplify_multi_(
        self, feature_data_list: list["FeatureData"], idxs_list: list[Indexes]
    ) -> "FeatureData":
        """
    ...
    def get_tensor_impl_(self, idxs: Indexes) -> torch.Tensor:
        ...
        return torch_cat(
            [
                feature_data.get_tensor(sub_idxs)
                for sub_idxs, feature_data in self.iterate(idxs)
            ],
            dim=0,
        )
    ...
    def to_indexes(self, idxs: Indexes | None) -> Indexes:
        """
...
class TensorFeatureData(FeatureData):
    ...
    def __init__(self, data: torch.Tensor, output_device: str | None = None):
        """
    ...
    def simplify_multi_(
        ...
        return TensorFeatureData(
            torch_cat(
                [
                    fd.data[idxs.get_idxs()]
                    for fd, idxs in zip(feature_data_list, idxs_list)
                ],
                dim=0,
            )
        )
...
class ConcatFeatureData(FeatureData):
    ...
    def simplify_impl_(self, idxs: Indexes) -> FeatureData:
        ...
        if len(simplified) == 1:
            ...
            if all_equal([type(fd) for fd in simplified]):
...
class SubsetFeatureData(FeatureData):
    ...
    def __init__(self, feature_data: FeatureData, idxs: Indexes):
        ...
        super().__init__(
            n_samples=len(idxs),
            device=feature_data.get_device(),
            dtype=feature_data.get_dtype(),
        )
        self.idxs = idxs
